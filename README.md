...
이 프로젝트는 `vercel.json`과 `api/` 서버리스 함수를 포함해 Vercel에도 올릴 수 있게 준비했습니다.

### 왜 구조가 다른가?

Vercel은 `python3 server.py`처럼 장기 실행 서버를 띄우는 방식보다, `api/search.py`와 `api/chart.py` 같은 **서버리스 함수**를 호출하는 방식에 맞습니다.
정적 파일(`index.html`, `style.css`, `app.js`)은 그대로 배포되고, 프론트엔드가 호출하는 `/api/search`, `/api/chart` 요청은 Vercel Python 함수가 처리합니다.

### Vercel에서 배포하는 방법

1. 이 폴더를 GitHub 저장소에 push 합니다.
2. Vercel 대시보드에서 **Add New → Project** 를 선택합니다.
3. `testStock` 저장소를 Import 합니다.
4. Framework Preset은 `Other` 그대로 두고 배포합니다.
5. 배포가 끝나면 Vercel이 발급한 URL로 접속합니다.
